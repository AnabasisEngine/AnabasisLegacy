...
public struct ManualResetAnabasisTaskSourceCore<TResult>
{
    ...
    private void ReportUnhandledError() {
        ...
        try {
            if (_error is OperationCanceledException oc) {
                AnabasisTaskScheduler.PublishUnobservedTaskException(oc);
            } else if (_error is ExceptionHolder e) {
                AnabasisTaskScheduler.PublishUnobservedTaskException(e.GetException().SourceException);
            }
        }
        catch {
            // ignored
    ...
    public bool TrySetResult(TResult result) {
        ...
        if (_continuation == null && Interlocked.CompareExchange(ref _continuation,
                ManualResetAnabasisTaskSourceCoreShared.Sentinel, null) == null) return false;
    ...
    /// <param name="token">Opaque value that was provided to the <see cref="AnabasisTask"/>'s constructor.</param>
    [DebuggerHidden]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public AnabasisTaskStatus GetStatus(short token) {
        ValidateToken(token);
        return _continuation == null || _completedCount == 0 ? AnabasisTaskStatus.Pending
            : _error == null ? AnabasisTaskStatus.Succeeded
            : _error is OperationCanceledException ? AnabasisTaskStatus.Canceled
            : AnabasisTaskStatus.Faulted;
    }
    ...
    public AnabasisTaskStatus UnsafeGetStatus() =>
        _continuation == null || _completedCount == 0 ? AnabasisTaskStatus.Pending
        : _error == null ? AnabasisTaskStatus.Succeeded
        : _error is OperationCanceledException ? AnabasisTaskStatus.Canceled
        : AnabasisTaskStatus.Faulted;
    ...
    public TResult GetResult(short token) {
        ...
        if (_completedCount == 0) {
            throw new InvalidOperationException("Not yet completed, AnabasisTask only allow to use await.");
        }
    ...
    public void OnCompleted(Action<object?> continuation, object? state, short token /*, ValueTaskSourceOnCompletedFlags flags */) {
        ...
        */
        ...
        if (!ReferenceEquals(oldContinuation, ManualResetAnabasisTaskSourceCoreShared.Sentinel)) {
