...
namespace Anabasis.Tasks
{
    ...
    /// </remarks>
    public class AnabasisProgress<T> : IProgress<T>
    {
        ...
        /// <summary>Initializes the <see cref="AnabasisProgress{T}"/>.</summary>
        public AnabasisProgress()
        {
            _invokeHandlers = InvokeHandlers;
        }

        /// <summary>Initializes the <see cref="AnabasisProgress{T}"/> with the specified callback.</summary>
        /// <param name="handler">
        ...
        /// the <see cref="AnabasisProgress{T}"/> at construction, it's possible that this handler instance
        /// could be invoked concurrently with itself.
        /// </param>
        /// <exception cref="System.ArgumentNullException">The <paramref name="handler"/> is null (Nothing in Visual Basic).</exception>
        public AnabasisProgress(Action<T> handler) : this()
        {
        ...
        protected virtual void OnReport(T value)
        {
            ...
            if (handler != null || changedEvent != null)
                AnabasisTaskScheduler.Schedule(AnabasisPlatformStepMask.All, _invokeHandlers, value);
        }
