...
public interface IAnabasisPlatform
{
    ...
    ///
    /// Any scoped services may assume that <see cref="Initialize"/> has already been called
    /// </summary>
    ...
    /// then all services required for the initialization of scoped services and <see cref="IAnabasisWindow"/> are available,
    /// if those services are not themselves already initialized by this method.
    /// Actually running the game loop infrastructure is implemented in <see cref="IAnabasisWindow.Run"/>
    /// </summary>
    public void Initialize();

    /// <summary>
    /// The platform window/view.
    /// Should be guaranteed non-null when <see cref="Initialize"/> returns.
    /// Will be registered as a singleton in the DI container, accessing before use of <see cref="Initialize"/> is an error.
    /// </summary>
